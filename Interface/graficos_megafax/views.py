...
from index.models import megafax
from datetime import datetime, timedelta
import matplotlib.pyplot as plt

def g_megafax(request): 

    datos = megafax.objects.all().order_by('nombre')
    dicc ={}
    for elementos in datos:
        dicc[elementos.nombre]=elementos.nombre.replace(" ","_")
    
    return render(request,'graf_megafax.html',{'datos':dicc})

def busqueda_m(request):

    canal_seleccionado = request.GET.get('canal')
    canal_seleccionado = canal_seleccionado.replace("_"," ")
    
    id_canal = megafax.objects.get(nombre=canal_seleccionado)
